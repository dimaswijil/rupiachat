...
class _WalletScreenState extends State<WalletScreen> {
  ...
  /// Inisialisasi Midtrans SDK dengan callback untuk transaksi
  void _initMidtrans() async {
    await _midtransService.initialize(
      onTransactionFinished: (TransactionResult result) {
        debugPrint('[Wallet] Transaksi selesai: ${result.transactionStatus}');
        ...
        if (result.isTransactionCanceled) {
          ...
          switch (result.transactionStatus) {
            ...
            case 'cancel':
            case 'expire':
              message = '❌ Pembayaran gagal: ${result.statusMessage ?? "Transaksi dibatalkan"}';
              bgColor = RupiaColors.danger;
              break;
            default:
              message = 'Status: ${result.transactionStatus ?? "Unknown"}';
              bgColor = RupiaColors.textSecondary;
          }
        }
        ...
        // Refresh saldo setelah transaksi selesai
        _loadBalance();
      },
    );
  }
                ...
                SizedBox(
                  ...
                  child: ElevatedButton(
                    ...
                    onPressed: isLoadingTopup ? null : () async {
                      ...
                      // 1. Ambil Snap Token dari Laravel
                      ...
                      if (snapToken != null) {
                        // 2. Buka Snap UI Midtrans dalam aplikasi
                        _midtransService.startPayment(snapToken);
                      } else {
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: const Text('Gagal membuat transaksi. Coba lagi.'),
